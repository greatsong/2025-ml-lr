...
def compute_yearly_mean(df_daily, target_col, miss_threshold=0.02):
    ...
    - 결측비율은 해당 연도 전체 일수(365/366) 대비
    - miss_ratio > miss_threshold 이면 NaN
...
if src == "CSV 업로드":
    ...
    if up is None:
        st.info("CSV 업로드 or 왼쪽에서 '기본' 선택"); st.stop()
...
# ---- 열 이름 정규화: '날짜' → 'date' 유지, 나머지는 한글 이름 그대로 사용 ----
df = df_raw.copy()
...
if hist_metric:
    ...
    chart_hist = alt.Chart(df.dropna(subset=[hist_metric])).mark_bar().encode(
...
# =========================
# 🌈 K-means 비지도 계절 구분 — 최저/최고 기준
# =========================
st.header("🌈 K-means 비지도 계절 구분 — 최저/최고기온 기반")

# 기본 컬럼명(한국어) 우선 사용, 없으면 드롭다운에서 선택
        ...
        if pd.isna(max_dt_all):
            ...
            if dfKc.empty:
                ...
                if "여름" in counts["season_unsup"].unique():
                    ...
                    st.info("K=4가 아니거나 데이터 특성상 '여름' 레이블이 없을 수 있습니다.")
...
st.markdown("""
**교육 메모**  
- 월별 박스플랏은 기본 12개월 전부, 선택 모드로 특정 월만 볼 수 있어요.  
- 회귀/예측은 **완전한 마지막 연도까지만** 학습·표시, 예측은 그 다음 해부터.  
- K-means는 **최저/최고기온**으로 계절을 자동 구분하고, **여름 일수 추세**로 길어지는지 확인합니다.
""")
